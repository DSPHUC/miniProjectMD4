...
getAllCompanies = async () => {
    ...
    companies.forEach((item) => {
        const str = renderCompanies(item, idCompany);
...
getAllPrices = async () => {
    ...
    let idPrices = 1;
    prices.forEach((item) => {
        const str = renderPrices(item, idPrices);
...
getAllColors = async () => {
    ...
    let idColor = 1;
    colors.forEach((item) => {
        const str = renderColors(item, idColor);
...
const filterProduct = async () => {
    ...
    const filteredProducts = products.filter(
        (item) =>
            ...
            (inputSearch == null || item.title.toLowerCase().includes(inputSearch.toLowerCase()))
            &&
